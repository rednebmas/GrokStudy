...
export function useWebRTC(onMessage: (message: Message) => void): UseWebRTCResult {
  ...
  const connect = useCallback(
    async (sampleRate: number) => {
      try {
        ...
        console.log(
          `[${getTimestamp()}] 📝 Creating WebRTC session with sample rate: ${sampleRate}Hz`,
        );
        ...
        console.log(
          `[${getTimestamp()}] ✅ Session created: ${data.session_id} with ${data.sample_rate}Hz`,
        );
